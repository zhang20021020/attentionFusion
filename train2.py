...
def export_vis_patches_256_with_heatmap(
    ...
    for tile_id in ids:
            ...
            for (x, y, w, h) in sliding_window(rgb_full,
                ...
                # =====【Grad-CAM 必须无 no_grad】=====
                logits, cam_rgb, cam_dsm = net(rgb_tensor, dsm_tensor, mode='VIS')
                cam_rgb = cam_rgb.squeeze(0).detach().cpu().numpy()  # [h,w], 0~1
                cam_dsm = cam_dsm.squeeze(0).detach().cpu().numpy()
                cam_rgb = cv2.resize(cam_rgb, (patch_size, patch_size))
                cam_dsm = cv2.resize(cam_dsm, (patch_size, patch_size))

                cam_rgb_u8 = (cam_rgb * 255).astype(np.uint8)
                cam_dsm_u8 = (cam_dsm * 255).astype(np.uint8)

                rgb_heat = cv2.applyColorMap(cam_rgb_u8, cv2.COLORMAP_JET)
                dsm_heat = cv2.applyColorMap(cam_dsm_u8, cv2.COLORMAP_JET)

                # # 7) 从 encoder 抽取 feature，用于热力图（注意：这是你 UNetFormer_TwoModal 的接口）
                # # low_feat, high_feat = net.encoder(rgb_tensor, dsm_tensor)   # low: B×C×H/8×W/8
                # # feat = low_feat  # 用高分辨率那一路做热力图
                # logits, rgb_feats, dsm_feats = net(rgb_tensor, dsm_tensor, mode='VIS')
                #
                # # 选尺度（强烈推荐 res1x / res1y，对应最高分辨率）
                # rgb_feat = rgb_feats[0]  # res1x
                # dsm_feat = dsm_feats[0]  # res1y
                # # ===== 构建 RGB / DSM 热力图 =====
                # rgb_heat = build_heatmap_from_feat(rgb_feat, (patch_size, patch_size))
                # dsm_heat = build_heatmap_from_feat(dsm_feat, (patch_size, patch_size))

                # ---- 生成热力图 ----
                # feat: [1, C, h, w]
                # feat_mean = feat.mean(dim=1, keepdim=True)               # 1×1×h×w
                # feat_up = F.interpolate(feat_mean, size=(patch_size, patch_size),
                #                         mode='bilinear', align_corners=False)  # 1×1×256×256
                # feat_np = feat_up.squeeze(0).squeeze(0).cpu().numpy()
                # feat_np = (feat_np - feat_np.min()) / (feat_np.max() - feat_np.min() + 1e-6)
                # feat_uint8 = (feat_np * 255).astype(np.uint8)            # 256×256

                # cv2 是 BGR，这里无所谓颜色精确与否，只是可视化
                # heat_color = cv2.applyColorMap(feat_uint8, cv2.COLORMAP_JET)  # 256×256×3
                ...
                # 把 dsm 灰度扩到 3 通道，方便叠加
                dsm_color = cv2.cvtColor(dsm_img, cv2.COLOR_GRAY2BGR)          # 256×256×3

                # 9) 叠加热力图（0.5/0.5 可以自己调）
                # rgb_heat = cv2.addWeighted(rgb_img, 0.5, heat_color, 0.5, 0)
                # dsm_heat = cv2.addWeighted(dsm_color, 0.5, heat_color, 0.5, 0)
                rgb_heat_overlay = cv2.addWeighted(rgb_img, 0.5, rgb_heat, 0.5, 0)
                ...
                rgb_heat_path = os.path.join(out_dir, f'{idx_str}_rgb_heat.png')
                dsm_heat_path = os.path.join(out_dir, f'{idx_str}_dsm_heat.png')
                rgb_heat_overlay_path= os.path.join(out_dir, f'{idx_str}_rgb_heat_overlay.png')
                dsm_heat_overlay_path= os.path.join(out_dir, f'{idx_str}_dsm_heat_overlay.png')
                ...
                cv2.imwrite(rgb_heat_path, rgb_heat)
                cv2.imwrite(dsm_heat_path, dsm_heat)
                cv2.imwrite(rgb_heat_overlay_path, rgb_heat_overlay)
                cv2.imwrite(dsm_heat_overlay_path, dsm_heat_overlay)
...
if MODE == 'Train':
    ...
    # 1) 加载你想可视化的权重
    if DATASET == 'Vaihingen':
        weight_path = 'F:\model\modelsv/UNetformer_epoch26_0.8484.pth'
    else:  # Potsdam
