...
# ===== Decoder family used by CIAPNet_Head and other utilities =====
...
class preAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        avgdots_normal = self.avgpool(avgdots_normal)
        avgdots_normal = torch.squeeze(avgdots_normal, dim=-1).contiguous()
        avgdots_normal = self.avgpool_arrange(avgdots_normal)
    ...
    def forward(self, x):
        for i in range(self.ca_num_heads):
            ...
            if i == 0:
                ...
                amask_out = torch.cat([amask_out, mask_i], -1)
        ...
        s_out = torch.chunk(s_out, self.ca_num_heads, dim=1)
        for i in range(self.ca_num_heads):
            ...
            if i == 0:
                ...
                Cattn_out = torch.cat([Cattn_out, attn_out], 1)
        return Cattn_out
...
class Detail_Recovery(nn.Module):
    ...
    def forward(self, res, attn):
        ...
        feature_map_pad = F.pad(qp, (4, 4, 4, 4), 'constant', 0)
        ...
        q = unfolded.reshape(B, C, H * W, 25).permute(0, 2, 3, 1).contiguous()

        point_aware = point_center @ q.transpose(-2, -1)
        point_aware = point_aware.index_fill_(-1, torch.LongTensor([12]).cuda(self.device), 0)
        point_aware_attn = point_aware.softmax(dim=-1)
        point_center_aware = point_aware_attn @ q
        ...
        k = attns.reshape(B, self.ca_num_heads, C, H * W, ).permute(0, 3, 1, 2).contiguous().reshape(B, H * W,
                                                                                                     self.ca_num_heads,
                                                                                                     C)
        CLASS_attn = point_center @ k.transpose(-2, -1)
        CLASS_attn = CLASS_attn.softmax(dim=-1)
        CLASS_attn_out = CLASS_attn.transpose(-2, -1) @ point_center
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        if self.use_cfpn:
            ...
            self.cfpn_gamma = nn.Parameter(torch.zeros(4))  # used when cfpn_blend == 'add'

        # Use CIAPNet_Head decoder
    ...
    def forward(self, x, y, mode='Train'):
        ...
        if self.use_cfpn:
            feats = [res1, res2, res3, res4]  # res1: highest resolution
            _, F_glb = self.cfa(feats)
            dist = self.cfd(F_glb)  # [Y1..Y4] align to [res1..res4]
